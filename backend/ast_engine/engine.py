...
def analyze_code(
    ...
    for d in detected:
        ...
        # If editable_ranges is None, treat as 'no filter applied' (whole file editable)
        # If editable_ranges is [] (empty list), nothing is editable — skip everything
        if editable_ranges is None:
            editable_detected.append(d)
        elif len(editable_ranges) == 0:
            # Empty list = no editable zones = nothing passes the filter
            pass
